...
} forEach [KPLIB_b_light_classes, KPLIB_b_heavy_classes, KPLIB_b_air_classes, KPLIB_b_light_camps_classes, KPLIB_b_heavy_camps_classes, KPLIB_b_air_camps_classes];
...
while { GRLIB_cleanup_vehicles > 0 } do {
...

    {
        ...
        if ( count _nearestfob == 3 ) then {
            if ( ( _nextvehicle distance _nearestfob > ( 1.2 * KP_liberation_FobRange ) ) && ( _nextvehicle distance startbase > ( 1.2 * KP_liberation_FobRange ) ) ) then {
